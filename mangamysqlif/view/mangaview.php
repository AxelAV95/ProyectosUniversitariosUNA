           ...
           <tbody id="manga-tabla">
               <?php
                    ...
                    foreach ($mangas as $current) {
                        ...
                        echo '<tr>';
                        echo '<td style="display:none">'.$current->getNombre().'</td>';
                         echo '<td style="display:none">'.$current->getAnio().'</td>';
    ...
    <script type="text/javascript">
        $(document).ready(function(){
          $("#filtrar").on("keyup", function() {
            var value = $(this).val().toLowerCase();
            $("#manga-tabla tr").filter(function() {
              $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
            });
          });
        });
        ...
        //     var rows = $("#manga-tabla").find("tr > td ").hide();
        //    // console.log(rows.children());
        //     var data = this.value.split(" ");
        //     $.each(data, function(i, v) {
        //         console.log(v);

        //       // console.log( rows.children().val());
        //       // rows.children().filter($(this).val().indexOf(v)).show();
        //         //rows.filter("input:contains('" + v + "')").show();
        //     });
        // });
    </script>
</body>
