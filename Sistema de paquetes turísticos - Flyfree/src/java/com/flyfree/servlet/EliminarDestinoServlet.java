...
public class EliminarDestinoServlet extends HttpServlet {

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {

        Destinos destino = new Destinos(0, "", "");
        destino.setDestinoid(Integer.parseInt(request.getParameter("codigo")));

        DestinosData eliminarDestino = new DestinosData();
        boolean elimino = false;
        try {
            elimino = eliminarDestino.eliminarDestino(destino);
        } catch (SQLException ex) {
            // ridereccionar al formulario para que verifique los datos ingresados
            Logger.getLogger(EliminarDestinoServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
